...
from opencv_cam_return_2 import Scanner
import cv2
import imutils
from PIL import Image
...
class Ui_MainWindow(object):
    ...
    def show_page(self, index):
        if 0 <= index < len(self.temp_png):
                #hasil=Scanner(self.temp_png[index])
                #scan    = Scanner(self.temp_png[index])
                #hasil   =scan.coba()
                #frame   = cv2.cvtColor(hasil, cv2.COLOR_BGR2RGB)
               # qimage  = QPixmap.fromImage(QImage(frame.data, frame.shape[1], frame.shape[0], frame.shape[1] * 3, QImage.Format_RGB888))
                #self.previewLabel.setPixmap(qimage)
               # self.previewLabel.setAlignment(QtCore.Qt.AlignCenter)
               pass


    def onClicked(self, index:int =1):
        scanner= Scanner('kamera')
        self.cap = scanner.inisiasi_awal()
        while (self.cap.isOpened()):
            ret, frame=scanner.camera_atau_gambar()
            _, frame=scanner.contour()
            if ret == True:
                self.displayImage(frame,1,index=1)
                cv2.waitKey()
            else:
    ...
    def displayImage(self, image, window=1, index:int=1):
        print(index)
    ...
    def open_camviewer(self, index:int=1):
