...
class Ui_Dialog(object):
    def processing(self, temp_png, isOCR):
        text=""
        pdf = []
        if isOCR == True:
            print(self.extComboBox.currentIndex())
            index = self.extComboBox.currentIndex()
            if index == 0:
                pdf_name, _ = QFileDialog.getSaveFileName(QtWidgets.QDialog(), "Save merged file", self.filenameEdit.text(), "PDF Files (*.pdf)")
                for i in range(len(temp_png)):
                    image = Image.open(temp_png[i])
                    pdf_temp = pytesseract.image_to_pdf_or_hocr(image, extension='pdf')
                    with open(f"temp_pdf/temp_{i}.pdf", 'wb') as f:
                        f.write(pdf_temp)
                    pdf.append(f"temp_pdf/temp_{i}.pdf")
                ...
                for i in range(len(temp_png)):
                    img = cv2.imread(temp_png[i])
                    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    adaptive_threshold = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 85, 35)
                    config = "--psm 4"
                    ocr_result = pytesseract.image_to_string(adaptive_threshold, config=config)
                    text += ocr_result + "\n" + "-" * 50 + "\n"
                txt_name, _ = QFileDialog.getSaveFileName(QtWidgets.QDialog(), "Save file", self.filenameEdit.text(), "TXT, (*.txt)")
                if txt_name:    
            ...
            if index == 1:
                for i in range(len(temp_png)):
                    image = Image.open(temp_png[i])
                    img_name = QFileDialog.getSaveFileName(QtWidgets.QDialog(), "Save file", self.filenameEdit.text(), "JPEG, (*.jpeg)")
                    image.save(img_name[0], "JPEG")
            if  index == 2:
                for i in range(len(temp_png)):
                    image = Image.open(temp_png[i])
                    img_name = QFileDialog.getSaveFileName(QtWidgets.QDialog(), "Save file", self.filenameEdit.text(), "PNG, (*.png)")
...
"}")
    ...
    def retranslateUi(self, Dialog):
        ...
        self.extComboBox.setItemText(3, _translate("Dialog", ".txt"))
